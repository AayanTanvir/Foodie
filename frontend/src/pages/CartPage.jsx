...
const CartPage = () => {
    ...
    let { cartItems, isCartEmpty } = useContext(CartContext);
    ...
    return (
        <div className="absolute top-0 left-0 w-full h-screen flex items-center justify-center flex-col pt-12">            <div className='w-[80%] h-[85%] border-2 border-gray-300 rounded-lg grid grid-rows-6 grid-cols-6'>
                <div className='row-start-1 row-end-7 col-start-1 col-end-5 p-6 grid auto-rows-auto grid-cols-3 overflow-y-auto gap-2'>
                    {cartItems.map((item) => (
                        <div
                        key={item.id}
                        className='w-full h-20 px-4 py-2 mb-5 border-2 border-gray-200 flex justify-between items-center rounded-xl cursor-pointer transition-transform duration-200 hover:scale-105 relative'
                        >
